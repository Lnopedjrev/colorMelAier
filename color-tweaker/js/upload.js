...
export function initUpload(elements, callbacks) {
  ...
  const { dropZone, fileInput, tsInput, btnLoad, buildChips, tsChips } = elements;
  ...
  fileInput.addEventListener('change', (e) => handleBuildFiles(e.target.files, btnLoad, buildChips));
  tsInput.addEventListener('change', (e) => handleTsFiles(e.target.files, tsChips));
...
async function handleBuildFiles(fileList, btnLoad, chipsEl) {
  ...
  chipsEl.innerHTML = Array.from(state.buildFileMap.keys())
    .slice(0, 30)
    .map(p => `<span class="file-chip">${p}</span>`)
    .join('') + (state.buildFileMap.size > 30
...
function rewriteUrlsInCss(css, blobMap) {
  let result = css.replace(/@import\s+url\(\s*['"]?([^'")]+)['"]?\s*\)/g, (match, ref) => {
    if (ref.startsWith('data:') || ref.startsWith('blob:') || ref.startsWith('http')) return match;
...
async function loadBuild() {
  ...
  if (!found) { alert('No index.html found in uploaded files'); return; }
  ...
  for (const [path, file] of state.buildFileMap) {
    ...
    const relativePath = htmlDir ? path.substring(htmlDir.length + 1) : path.substring(1);
  ...
  doc.querySelectorAll('link[rel="manifest"], script[src*="ngsw"]').forEach(el => el.remove());
  ...
  doc.querySelectorAll('script[type="module"]').forEach(() => { hasModuleScripts = true; });
  ...
  if (hasModuleScripts && Object.keys(importEntries).length > 0) {
    ...
    console.log('[CT] import map:', Object.keys(importEntries).length / 2, 'modules');
  }
  ...
  listenerEl.textContent = 'window.addEventListener("message",function(e){' +
